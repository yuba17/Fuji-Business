...
@if(auth()->user()->isDirector() || auth()->user()->isManager() || (auth()->user()->isTecnico() && $task->assigned_to === auth()->id()))
    <x-ui.button href="{{ route('tasks.edit', $task) }}" variant="secondary">
...
<div class="mb-8">
    ...
    <div class="flex flex-wrap gap-4 text-sm text-gray-600">
        ...
        @endif
        @if($task->due_date)
            <div class="flex items-center gap-2 {{ $task->isOverdue() ? 'text-red-600 font-semibold' : '' }}">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    @if($task->subtasks->count() > 0)
        <x-ui.card>
            <h2 class="text-xl font-bold text-gray-800 mb-4">Subtareas</h2>
            <div class="space-y-2">
                @foreach($task->subtasks as $subtask)
                    <div class="border-l-4 border-blue-500 pl-4 py-2">
                        <h3 class="font-semibold text-gray-900">{{ $subtask->title }}</h3>
                        <div class="flex items-center gap-2 mt-1">
                            <x-ui.badge variant="{{ $subtask->status === 'done' ? 'success' : 'warning' }}">
                                {{ $subtask->status_label }}
                            </x-ui.badge>
                            <a href="{{ route('tasks.show', $subtask) }}" class="text-xs text-red-600 hover:text-red-700">
                                Ver →
                            </a>
                        </div>
                    </div>
